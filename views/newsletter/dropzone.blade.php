<script>
	 ...
	 * Newsletter Images
	 */
	jQuery('.dropzone-img').dropzone({ //'#dropzone-uploader'
		previewsContainer: $('.dropzone-img-preview')[0],
		previewTemplate: $('.dropzone-img-template').html(),
		clickable: '.dz-img-clickable',
		url: '{{ route('newsletter.file.store', $resource_id) }}',
		paramName: 'file', // The name that will be used to transfer the file
	    maxFilesize: 5, // MB
		...
		success: function (file, response) {
			var $preview = $(file.previewElement);
			var deleteLink = $preview.find('.img-delete')[0].dataset.href.replace(/\/1/g, '/' + response.id);
			$preview.find('.delete-button').remove();
			$preview.find('.progress').hide();
			$preview.find('.completed').hide();
			$preview.find('.uploading').remove();
			$preview.find('.dz-error-message').remove();
			$preview.find('.dz-img-id').attr('value', response.id);
			$preview.find('.img-delete').attr('href', deleteLink);
		},
		error: function (file, response) {
			var $preview = $(file.previewElement);
			$preview.find('.img-delete').remove();
			$preview.find('.delete-button').show();
			$preview.find('.image').remove();
			$preview.addClass('negative message');
			$preview.find('.uploading').remove();
			$preview.find('.progress').hide();
		...
		queuecomplete: function() {
	    	...
	    	$('.dropzone-img').removeClass('loading');
		}
		// removedfile: function(file) {
		// 	$.ajax({
		// 		data: {
